...
def run_evaluation(input_data, target_data):
    ...
    for feed_indices, testing_indices in tscv.split(input_data):
        ...
        for j in range(len(testing_target_data)):
            model_aes.append(abs(testing_target_data[j] - model_testing_predictions[j]))
        ...
        model_mae = stats.mean(model_aes)
        ...
        print("LBFGS Model MAE - " + str(lbfgs_model_mae))
        print("Adam Model MAE - " + str(adam_model_mae))
        ...
        i += 1
    model_mean_mae = stats.mean(model_maes_across_splits)
    ...
    model_mean_rmse = stats.mean(model_rmses_across_splits)
    ...
    model_mean_nrmse = stats.mean(model_nrmses_across_splits)
    ...
    print("Model MAEs Across Splits - " + str(model_maes_across_splits))
    print("LBFGS Model MAEs Across Splits - " + str(lbfgs_model_maes_across_splits))
    print("Adam Model MAEs Across Splits - " + str(adam_model_maes_across_splits))
    print("Model RMSEs Across Splits - " + str(model_rmses_across_splits))
    print("LBFGS Model RMSEs Across Splits - " + str(lbfgs_model_rmses_across_splits))
    print("Adam Model RMSEs Across Splits - " + str(adam_model_rmses_across_splits))
    print("Model N-RMSEs Across Splits - " + str(model_nrmses_across_splits))
    print("LBFGS Model N-RMSEs Across Splits - " + str(lbfgs_model_nrmses_across_splits))
    print("Adam Model N-RMSEs Across Splits - " + str(adam_model_nrmses_across_splits))
    ...
    print("Model Mean MAE - " + str(model_mean_mae))
    print("LBFGS Model Mean MAE - " + str(lbfgs_model_mean_mae))
    print("Adam Model Mean MAE - " + str(adam_model_mean_mae))
...
def calculate_total_weighted_feature_importances(model):
    ...
    for i in range(model.get_model_depth()):
        total_weighted_input_importances_across_layers.append([])
    for i in reversed(range(model.get_model_depth())):
        layer_total_weighted_input_importances_across_neurons = []
        for j in range(int(math.pow(model.get_neuron_size_base(), model.get_model_depth() - 1 - i))):
